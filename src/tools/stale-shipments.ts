...
export function registerStaleShipmentTools(server: McpServer): void {
  server.tool(
    "ecount_shipment_stale_shipments",
    "N일 이상 상태가 갱신되지 않은 선적 건을 식별합니다.",
    {
      days: z.number().default(7).describe("미갱신 기준 일수 (기본 7)"),
    },
    { readOnlyHint: true },
  ...
  server.tool(
    "ecount_shipment_customs_delays",
    "입항 후 통관이 지연되고 있는 선적 건을 식별합니다. (status=arrived, N일 초과)",
    {
      maxDays: z.number().default(7).describe("통관 지연 기준 일수 (기본 7)"),
    },
    { readOnlyHint: true },
    async (params: Record<string, unknown>) => {
      try {
  ...
  server.tool(
    "ecount_shipment_delivery_delays",
    "통관 완료 후 배송이 지연되고 있는 선적 건을 식별합니다. (status=cleared, N일 초과)",
    {
      maxDays: z.number().default(3).describe("배송 지연 기준 일수 (기본 3)"),
    },
    { readOnlyHint: true },
    async (params: Record<string, unknown>) => {
      try {
