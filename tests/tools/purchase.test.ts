...
import { registerPurchaseTools } from "../../src/tools/purchase.js";
...
describe("registerPurchaseTools", () => {
  it("should register all 7 purchase tools without throwing", () => {
    const server = new McpServer({ name: "test", version: "0.1" });
    const client = new EcountClient(mockConfig);
    expect(() => registerPurchaseTools(server, client)).not.toThrow();
  });
});
